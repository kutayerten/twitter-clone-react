...
export default function Search() {
  ...
  return (
    <div ref={ref} className='min-h-[32px] h-[53px] mb-3 flex items-center sticky top-0 bg-[color:var(--background-primary)] z-10'>
        <label className='h-[43px] rounded-full  bg-[color:var(--background-third)] w-full relative group focus-within:bg-[color:var(--background-primary)] border border-transparent focus-within:border-[#1d9bf0]'>
            <div className="w-[56px] h-full flex items-center justify-center absolute top-0 left-0 pointer-events-none">
                <svg viewBox="0 0 24 24" height={18.75} className=' min-w-[32px] text-[color:var(--color-base-secondary)]  group-focus-within:text-[color:var(--color-base-primary)] '>
                    <path
                    fill='currentColor'
            ...
            <input type="text"
            placeholder='Search'
            className='w-full h-full bg-transparent p  rounded-full outline-none pl-[56px] text-[15px]' 
            value={query}
        ...
        {focus &&(
                <div className="absolute -translate-y-1 top-full w-[350px] bg-[color:var(--background-primary)] shadow-box max-h-[calc(80vh-53px)] rounded-lg text-center min-h-[100px]">
